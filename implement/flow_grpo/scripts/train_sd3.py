import os
# Set NCCL timeout BEFORE any torch/distributed import
os.environ["NCCL_TIMEOUT"] = "3600000"  # 60 min in ms
...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        for key, value in rewards.items():
            rewards_gather = accelerator.gather(torch.as_tensor(value, device=accelerator.device).float()).cpu().numpy()
...
def main(_):
    ...
    if config.use_lora:
        ...
        # Full param: bf16 for memory efficiency
        ...
        if getattr(config, 'activation_checkpointing', False):
            ...
            logger.info("[Full param] bf16 + gradient checkpointing")
    ...
    # LR warmup scheduler
    warmup_steps = getattr(config.train, 'lr_warmup_steps', 0)
    if warmup_steps > 0:
        ...
        logger.info(f"LR warmup: {warmup_steps} steps, target lr={config.train.learning_rate}")
    else:
        lr_scheduler = None
    ...
    while True:
        ...
        # === Console print for monitoring ===
        if accelerator.is_main_process:
            ori_avg = gathered_rewards.get("ori_avg", gathered_rewards.get("avg", None))
            if ori_avg is not None:
                import numpy as _np
                r = ori_avg.flatten() if hasattr(ori_avg, 'flatten') else _np.array(ori_avg).flatten()
                print(f"[Epoch {epoch}] step={global_step} | reward={r.mean():.4f}±{r.std():.4f} (min={r.min():.3f} max={r.max():.3f})", flush=True)
        ...
        # Skip epoch if no valid samples after filtering
        if total_batch_size == 0:
            if accelerator.is_main_process:
                print(f"[WARNING] Epoch {epoch}: no valid samples (all advantages=0), skipping training", flush=True)
            epoch += 1
            continue
        # assert (
        #     total_batch_size
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            # rebatch for training (handle case where batch is smaller than expected)
            actual_batches = max(1, total_batch_size // max(1, config.sample.num_batches_per_epoch))
            samples_batched = {
                k: v[:actual_batches * config.sample.num_batches_per_epoch].reshape(
                    -1, actual_batches, *v.shape[1:]
                ) if total_batch_size >= actual_batches else v.unsqueeze(0)
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        if accelerator.sync_gradients:
                            # Print grad norm BEFORE clipping (for monitoring)
                            if accelerator.is_main_process:
                                _raw_grad = torch.nn.utils.clip_grad_norm_(transformer.parameters(), float('inf'))
                                info["raw_grad_norm"] = [_raw_grad]
                    ...
                    if accelerator.sync_gradients:
                        ...
                        if accelerator.is_main_process:
                            ...
                            # Console print key training metrics
                            _loss = info.get("loss", torch.tensor(0)).item() if hasattr(info.get("loss", 0), 'item') else info.get("loss", 0)
                            _clip = info.get("clip_fraction", torch.tensor(0)).item() if hasattr(info.get("clip_fraction", 0), 'item') else info.get("clip_fraction", 0)
                            _kl = info.get("approx_kl", torch.tensor(0)).item() if hasattr(info.get("approx_kl", 0), 'item') else info.get("approx_kl", 0)
                            _lr = optimizer.param_groups[0]['lr']
                            _clipfrac = info.get("clipfrac", torch.tensor(0)).item() if hasattr(info.get("clipfrac", 0), 'item') else 0
                            _approx_kl = info.get("approx_kl", torch.tensor(0)).item() if hasattr(info.get("approx_kl", 0), 'item') else 0
                            _grad = info.get("raw_grad_norm", [torch.tensor(0)])[0].item() if hasattr(info.get("raw_grad_norm", [torch.tensor(0)])[0], 'item') else 0
                            print(f"  [Train] step={global_step} | loss={_loss:.4f} clip={_clipfrac:.3f} kl={_approx_kl:.6f} lr={_lr:.2e} grad={_grad:.4f}", flush=True)
                        global_step += 1
                        info = defaultdict(list)
                if config.train.ema:
